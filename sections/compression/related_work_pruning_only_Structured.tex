...
\section{Introduction}
The previous two chapters surveyed the landscape of Transformer pruning, dividing the state-of-the-art into Unstructured/Heterogeneous methods (Chapter 4) and Structured methods (Chapter 5). While each method claims significant compression capabilities, they optimize different units, rely on different recovery pipelines, and target different deployment metrics. This chapter synthesizes this literature by comparing their empirical results side-by-side. Furthermore, it critically analyzes the current state-of-the-art to identify recurring flaws in evaluation protocols, recovery budgets, and calibration sensitivities. Most importantly, this chapter isolates the fundamental flaw shared by nearly all reviewed methods—the reliance on local scoring at the expense of global allocation—setting the stage for the proposed contribution of this thesis.
