...
\subsection{Foundational Low-Rank Methods}
...
\subsubsection{ALBERT: Factorized Embeddings and Parameter Sharing \cite{lan2019albert}}
...
\subsubsection{LoRA: Low-Rank Adaptation \cite{hu2021lora}}
...
\begin{equation}
  \mathbf{h} = \W_0\mathbf{x} + \lambda\,\B\A\mathbf{x},
\end{equation}
where $\lambda$ is a scalar scaling factor (original LoRA uses $\lambda=\frac{\alpha}{r}$ in practice).

\paragraph{Key Theoretical Insights}
The LoRA paper introduced several important observations:
\begin{itemize}
  \item The \emph{intrinsic rank} hypothesis: the dominant directions required to adapt pre-trained LMs occupy a low-dimensional subspace; therefore direct low-rank updates are sufficient to capture task-specific weight changes.
  \item Practical implementation injects $\B,\A$ at selected dense projections, such as query, key, value, and MLP layers. The base matrix $\W_0$ remains frozen, while only $\B,\A$ are updated. This sharply reduces trainable parameter count while preserving comparable downstream performance.
\end{itemize}
...
\subsubsection{PiSSA: Principal Singular Values and Vectors Adaptation \cite{meng2024pissa}}
...
\subsubsection{rsLoRA: Rank-Stabilized LoRA \cite{kalajdzievski2023rslora}}
...
\subsection{Structural and Decoupled Methods}
...
\subsubsection{DoRA: Weight-Decomposed Low-Rank Adaptation \cite{liu2024dora}}
\label{subsubsec:dora}

DoRA decomposes a pretrained weight matrix into per-column magnitudes and unit-norm directions. Writing $\W$ column-wise as $\W=[\mathbf{w}_1,\dots,\mathbf{w}_k]$:
\begin{equation}
  \mathbf{w}_j = m_j \cdot \mathbf{v}_j,\qquad m_j\in\mathbb{R},\ \|\mathbf{v}_j\|_2=1.
\end{equation}
DoRA freezes the pre-trained direction $\mathbf{v}_{j}^{(0)}$ and updates direction with a low-rank correction while training a small magnitude adjustment vector $\m$:
\begin{equation}
...
\subsubsection{VeRA: Vector-Based Random Matrix Adaptation \cite{kopiczko2023vera}}
...
\begin{equation}
  \mathbf{h}^{(l)} = \W_0^{(l)}\mathbf{x} + \Lambda_b^{(l)}\,\B_{\text{shared}}\,\Lambda_d^{(l)}\,\A_{\text{shared}}\,\mathbf{x}.
\end{equation}
...
\subsection{Adaptive Rank Allocation Methods}
...
\subsubsection{AdaLoRA: Adaptive Budget Allocation  \cite{zhang2023adalora}}
...
\subsubsection{DyLoRA: Search-Free Dynamic Adaptation \cite{valipour2022dylora}}
...
\subsubsection{SoRA: Sparse Low-Rank Adaptation \cite{ding2023sora}}
...
\subsubsection{GaLore: Gradient Low-Rank Projection \cite{zhao2024galore}}
...
\begin{table}[H]
  \centering
  \caption{GLUE Average Scores on DeBERTaV3-base. Slight variations due to experimental setups. Sources: \cite{meng2024pissa}, \cite{zhang2023adalora}, \cite{ding2023sora}.}
...
\paragraph{Key Observations}
GaLore achieves near-identical performance to full-rank training (14.65 vs 14.61 perplexity) while reducing memory by 31\%. In contrast, standard LoRA shows significant performance degradation when used for pre-training rather than fine-tuning, indicating that gradient projection methods such as GaLore are better suited to training from scratch. Within this comparison, LoRA-family approaches remain more effective for adaptation, whereas GaLore supports memory-efficient pre-training.
