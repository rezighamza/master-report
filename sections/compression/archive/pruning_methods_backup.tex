...
\subsubsection{Weight-Magnitude and Gradient-Based Pruning \cite{han2015deep}}
...
\subsubsection{Wanda: Activation-Aware Post-Training Pruning \cite{sun2023simple}}
...
\subsubsection{SparseGPT: Second-Order One-Shot Pruning \cite{frantar2023sparsegpt}}
...
\begin{itemize}
	\item \textbf{Attention Heads:} In Multi-Head Attention (MHA) with $h$ heads, removing a head eliminates an entire parallel attention computation path. For model dimension $d$ and head dimension $d_h = d/h$, removing one head saves $\approx 4d \cdot d_h$ parameters in $W_Q, W_K, W_V, W_O$ matrices \cite{Michel2019}.
...
\subsubsection{LLM-Pruner: Gradient-Based Dependency Discovery \cite{ma2023llmpruner}}
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\RETURN Pruned model $M'$ with LoRA adapters
	\end{algorithmic}
\end{algorithm}
...
\paragraph{SynFlow (Synaptic Flow). \cite{tanaka2020pruning}}
...
\paragraph{Zen-Score (Network Expressivity). \cite{lin2021zen}}
...
AZ-NAS \cite{lee2024aznasassemblingzerocostproxies} improves ranking correlation by assembling multiple ZCPs that probe complementary architectural traits. For a network with $L$ primary blocks, let $f_l \in \mathbb{R}^{c \times n}$ denote block $l$ outputs and $g_l \in \mathbb{R}^{c \times n}$ denote block $l$ gradients.
...
\paragraph{T-Razor: Transformer-Specific Zero-Cost Proxies. \cite{zhou2022trazor}}
...
Importance scores, e.g., for a feed-forward neuron \(i\) in layer \(l\), are computed as \cite{Michel2019}:
\[
...
\subsubsection{EvoPress: Evolutionary Search for Heterogeneous Compression \cite{evopress2024}}
...
\subsubsection{DarwinLM: Training-Aware Evolutionary Pruning \cite{darwinlm2025}}
