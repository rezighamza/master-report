...
\section{Conclusion}
This chapter reviewed pruning methods that prioritize maximum theoretical flexibility and search space exploration: Unstructured and Heterogeneous Search-based methods. Unstructured methods such as Wanda and SparseGPT primarily optimize weight-level saliency, while search-based methods such as EvoPress and DarwinLM treat pruning as configuration optimization under explicit	 budget or hardware constraints. The next chapter will review structured methods that optimize for dense-shape acceleration through joint pruning of executable structures such as attention heads, FFN channels, and layers. The structured methods will be organized by their optimization signal, with families of differentiable mask-learning (CoFi), dependency-aware gradient methods (LLM-Pruner), Fisher-based post-training frameworks, and runtime-aware second-order methods (ZipLM). The empirical results will then be compared across all methods by pruning regime, with separate sections for perplexity retention, downstream task accuracy, measured speedup, and recovery cost.