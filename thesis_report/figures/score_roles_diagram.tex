...
% -----------------------------------------------------------------------------
% Layout arithmetic, chosen to keep the figure wide, readable, and non-cramped.
% Coordinate system: 1 TikZ unit = 1 cm.
% Total drawing span is roughly 25.9 cm x 6.9 cm.
%
% Horizontal blocks:
%   left sources:  x = 0.00..4.35
%   gate block:    x = 5.55..9.65     gap from sources = 1.20
%   heatmaps:      x = 10.75..18.85   gap from gate = 1.10
%   roles panel:   x = 20.05..25.65   gap from plots = 1.20
%
% Heatmap cells:
%   s_H: 4 x 4, cell = 0.56 cm, width = 2.24 cm
%   s_N: 4 x 10, cell = 0.44 cm, width = 4.40 cm
% -----------------------------------------------------------------------------
...
\node[font=\fontsize{13.8}{15}\selectfont, anchor=south] at ($(\hX+1.12,\hY+0.27)$) {$s_H[\ell,h]$};
...
\draw[arr] ($(\hX+1.12,\hY-2.24)$) -- ($(\hX+1.12,2.92)$);
\draw[arr] ($(\nX+2.20,\nY-1.76)$) -- ($(\nX+2.20,2.92)$);
...
\node[font=\fontsize{13.4}{15}\selectfont, anchor=south] at ($(\bX+1.43,\bY+\bH+0.14)$) {$imp(\ell)$};
...
\node[font=\fontsize{12.5}{14}\selectfont, anchor=south west] at ($(\cX,\cY+\cH+0.14)$) {$S_{\ell}[b]$ cumulative block table};

% Draw line by explicit cumulative values. Mapping: y = cY + 0.13 + ys * value.
\coordinate (p0)  at ($(\cX+0*\dx,\cY+0.13+0.00*\ys)$);
