...
\begin{tikzpicture}[
  ...
  actnode/.style={rectangle,draw=black!82,line width=0.95pt,rounded corners=2pt,minimum width=2.15cm,minimum height=0.96cm,inner sep=0pt,fill=white},
...
\node[textnode,anchor=west] at ($(b.east)+(0.22,0.02)$) {Bias};
\draw[arr] (b.south) -- ($(sum.north)+(0,0.02)$);

% ---------------- activation and output ----------------
\node[actnode] (act) at (\xAct,0) {\Large $f(\cdot)$};
\draw[arr] (sum.east) -- (act.west);

\node[mathlab,anchor=south] at ($(sum.east)!0.95!(act.west)+(0,0.5)$)
  {$z=\displaystyle\sum_{i=1}^{n}\omega_i x_i-b$};

\draw[arr] (act.east) -- (\xOut,0);
\node[mathlab,anchor=south] at ($(act.east)!0.59!(\xOut,0)+(0,0.15)$) {$y=f(z)$};
\node[textnode,anchor=west,font=\Large] at (\xOut+0.08,0.10) {$y$};
