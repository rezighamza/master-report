% Rebuilt from the uploaded HTML/SVG figure as a printable TikZ figure.
...
\definecolor{lineGrey}{HTML}{AEB7C2}
\definecolor{cellFill}{HTML}{EDF1F5}
\definecolor{softBlue}{HTML}{EEF4FA}
\definecolor{softSand}{HTML}{F7F2E8}
\definecolor{selectBlue}{HTML}{6F8FC9}
\definecolor{selectBlueDark}{HTML}{46589A}
\definecolor{pruneRed}{HTML}{C85C48}
\definecolor{pruneRedDark}{HTML}{8F3E31}
\definecolor{keepTeal}{HTML}{5DA399}
\definecolor{keepTealDark}{HTML}{246A63}
\definecolor{hessGold}{HTML}{D8B56D}
\definecolor{hessGoldDark}{HTML}{8A6730}
\definecolor{violetSoft}{HTML}{F0EFF8}
...
\tikzset{
  ...
  arrow/.style={-{Stealth[length=3.5mm,width=2.7mm]}, line width=0.9pt, draw=mutedText},
...
\begin{scope}[shift={(T0)}]
  \node[font=\Large\itshape, text=selectBlueDark] at (0.78,3.45) {$W$};
  \def\s{0.58}
  \def\g{0.075}
  \def\x0{2.05}
  \def\y0{1.75}
  ...
  \node[font=\bfseries\small, text=pruneRedDark] at ({\tx+0.5*\s},{\ty+0.5*\s}) {$w_{ij}$};
  \draw[pruneRed, line width=0.6pt] ({\tx+0.5*\s},{\y0+5*(\s+\g)+0.05}) -- ++(0,0.42);
  \node[font=\small\bfseries, text=pruneRedDark] at ({\tx+0.5*\s},5.20) {column $j$};
...
\begin{scope}[shift={(T1)}]
  ...
  \node[font=\Large\itshape, text=hessGoldDark] at (1.00,3.48) {$H^{-1}$};
  \def\s{0.50}
  \def\g{0.055}
  \def\x0{1.78}
  \def\y0{1.55}
  ...
  \node[font=\scriptsize\bfseries, text=hessGoldDark] at ({\dx+0.5*\s},{\dy+0.5*\s}) {$s_j$};
  % vector extraction
  ...
  \node[font=\small\bfseries, text=hessGoldDark] at (6.43,4.12) {$d_j$};
...
\begin{scope}[shift={(T2)}]
  ...
  \node[font=\bfseries\small, text=keepTealDark] at (5.85,3.34) {$+\delta w$};
...
\begin{scope}[shift={(B0)}]
  ...
  \node[font=\small\bfseries,text=selectBlueDark] at ({\gx+2*\s+1.5*\g},1.28) {$M=4$ group};
  \node[font=\small\bfseries,text=pruneRedDark] at ({\gx+2*\s+1.5*\g},1.00) {$N=2$ selected jointly};
  \node[smallnote,text width=7.1cm] at (4.1,0.45) {The mask is chosen for the whole group,\\not one scalar at a time.};
\end{scope}

% ===================== BOTTOM PANEL 2 =====================
\begin{scope}[shift={(B1)}]
  \node[formula] at (4.1,5.34) {$H_G^{-1} = [H^{-1}]_{G,G}\in\mathbb{R}^{M\times M}$};
  \node[font=\Large\itshape, text=hessGoldDark] at (0.95,3.42) {$H^{-1}$};
  \def\s{0.43}
  \def\g{0.050}
  \def\x0{1.90}
  \def\y0{1.52}
  ...
  \node[font=\small\bfseries,text=hessGoldDark] at (7.03,3.85) {$4\times4$};
...
\begin{scope}[shift={(B2)}]
  ...
  \foreach \c in {2,4}{
    ...
    \node[font=\scriptsize\bfseries,text=keepTealDark] at ({\xx+0.5*\s},{\ry+0.5*\s}) {$+\delta$};
  }
  \node[formula,text width=7.65cm, font=\scriptsize] at (4.1,4.72)
    {$\displaystyle \delta w_G=-H_{G,S}^{-1}\big([H_G^{-1}]_{SS}\big)^{-1}w_S$};
  \node[font=\scriptsize\bfseries,text=keepTealDark] at (6.35,3.08) {joint correction};
  ...
  \end{scope}
  \node[smallnote,text width=7.2cm] at (4.1,0.46) {Regular 2:4 mask;\\correction computed jointly.};
\end{scope}
